...
def create_template(query):
    ...
    return messages
def prepare_full_query(query):
    query = truncate_string(normalize_text(query), 200)
    chat_template = f"{tokenizer.apply_chat_template(create_template(query), tokenize=False, add_generation_prompt=True)}"
    return chat_template

def process_response(ql, res):
    completions = res.split("##query##:")
    return completions[-1][ql+2:]

def tokenize(input, max_length):
    templatised_input = []
    # print(type(input) , input[0], type(input[0]))
    for index in input:
        trimmed_query = prepare_full_query(index)
        templatised_input.append(trimmed_query)
        # print("TRIMMED_QUERY _ ", trimmed_query)
    return_dict = tokenizer.batch_encode_plus(templatised_input, truncation=True, 
            ...
            for status, prefix, _ in stream: 
                ...
                if len(batches)==args.batch_size:
                    ...
                    for i in range(len(res)):
                        res[i] = process_response(len(batches[i]),res[i])
